  ...
  try {
    ...
    if (userExist) {
      return res
        ...
        .json({
          message:
            "Username is already taken by a registered user. Please choose another.",
        });
    }
    ...
    return res.status(201).json({
      ...
      expiresAt,
      message:
        "Guest session started. You can use random text chat until you logout or the session expires.",
    });
  } catch (error) {
    console.error("Guest login failed:", error);
    return res
      .status(500)
      .json({ message: "Guest login failed, try again later." });
  }
};
  ...
  try {
    ...
    if (guestSessionId) {
      ...
      if (!session) {
        return res
          .status(400)
          .json({ message: "Invalid or expired guest session id." });
      }
      ...
      if (userExist) {
        ...
        return res
          .status(400)
          .json({ message: "username already taken by a registered user." });
      }
      ...
      const userExist = await User.findOne({
        username: usernameProvided.trim(),
      });
      if (userExist)
    ...
    return res.status(201).json({
      message:
        "Registered. Please verify your email to enable permanent login.",
    });
  } catch (error) {
...
exports.verifyEmail = async (req, res) => {
  try {
    ...
    // If the token is not found, check if it's because the user is already verified
    ...
    if (user.isVerified) {
      return res
        .status(409)
        .json({ message: "Email already verified successfully!." });
    }
    ...
    return res
      .status(500)
      .json({
        message: "Something went wrong. Please try again email verification.",
      });
  }
};
...
exports.resendVerification = async (req, res) => {
  try {
    ...
    const verificationLink = `${process.env.FRONTEND_URL}/email-verified/${token}`;
    await mailSender(
      user.email,
